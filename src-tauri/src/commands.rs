use crate::domain::{AppResult, AppError, HistoryListResponse, HistoryItem, OverviewStats};
...
#[tauri::command]
pub fn list_history(page: u32, page_size: u32, filters: HistoryFilters) -> AppResult<HistoryListResponse> {
    if page_size == 0 || page_size > 500 { return Err(AppError::Invalid("page_size out of range".into())); }
    ...
    if let Some(kw) = &filters.keyword { 
        if !kw.is_empty() { 
            where_clauses.push(format!("(title LIKE ?{} OR url LIKE ?{})", param_index, param_index + 1)); 
    ...
    let where_sql = if where_clauses.is_empty() { "".into() } else { format!("WHERE {}", where_clauses.join(" AND ")) };
    ...
    let total: i64 = with_conn(|conn| {
        ...
        let val: i64 = stmt.query_row(rusqlite::params_from_iter(params_dyn.iter()), |r| r.get(0))?;
        Ok(val)
    })?;
...
pub fn stats_overview(time_range: Option<String>) -> AppResult<OverviewStats> {
    ...
    let (total_visits, distinct_sites) = with_conn(|conn| {
        let total_sql = format!("SELECT SUM(num_visits) FROM navigation_history {}", where_sql);
        ...
        let total: i64 = if params.is_empty() {
            conn.query_row(&total_sql, [], |r| r.get::<_, Option<i64>>(0))?.unwrap_or(0)
        } else {
            conn.query_row(&total_sql, rusqlite::params_from_iter(params.iter()), |r| r.get::<_, Option<i64>>(0))?.unwrap_or(0)
        };
        
        let distinct: i64 = if params.is_empty() {
            conn.query_row(&distinct_sql, [], |r| r.get(0))?
        } else {
            conn.query_row(&distinct_sql, rusqlite::params_from_iter(params.iter()), |r| r.get(0))?
        };
    ...
    let top_entities: Vec<String> = with_conn(|conn| {
        ...
        while let Some(row) = rows.next()? { 
            ...
            if let Some(s) = id { acc.push(s); } 
        }
        Ok::<_, rusqlite::Error>(acc)
    })?;

    Ok(OverviewStats { total_visits, distinct_sites, top_entities })
}
...
pub fn set_db_path(path: String) -> AppResult<String> {
    ...
    config.set_db_path(path.clone())
        .map_err(|e| AppError::Internal(e.to_string()))?;
    
    // 重新初始化数据库连接
    crate::db::reset_connection(&path)
